...
namespace GLaDOSV3.Module.Default
{
    ...
    public class ModeratorModule : ModuleBase<ShardedCommandContext>
    {
        ...
        [RequireMfa]
        public async Task Purge(int numberOfMessages = 20)
        {
        ...
        [RequireBotPermission(GuildPermission.ManageMessages)]
        [RequireMfa]
        public async Task Prune(IUser mention, int numberOfMessages = 20)
        {
            if (numberOfMessages < 2)
            {
                await this.ReplyAsync("**ERROR: **Please Specify the amount of messages you want to clear").ConfigureAwait(false);
        ...
        public async Task Ban(ulong userid, [Remainder]string reason = "Unspecified.")
        {
            ...
            try
            {
                if (normalUser != null) await normalUser.SendMessageAsync($"You were banned from {Context.Guild.Name} for \"{reason}\" by {Context.Client.CurrentUser.Username}#{Context.Client.CurrentUser.Discriminator}").ConfigureAwait(false);
                await Context.Guild.AddBanAsync(userid, 0, $"Banned by {Context.Message.Author.Username}#{Context.Message.Author.Discriminator} | Reason: {reason}").ConfigureAwait(false);
                if (normalUser != null)
                {
                    if (!silent)
                ...
                {
                    if (!silent)
                        await this.ReplyAsync($"Goodbye random person!").ConfigureAwait(false);
                    else
                        await ((IDMChannel)Context.Message.Author.CreateDMChannelAsync().GetAwaiter().GetResult()
                                                  .SendMessageAsync($"Begone random person!").GetAwaiter().GetResult().Channel)
                             .CloseAsync().ConfigureAwait(false);
